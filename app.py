...
def index():
    ...
    for root, _, files in os.walk(PDF_DIR):
        ...
        if fichiers_pdf:
            fichiers_pdf.sort(key=lambda f: os.path.getmtime(os.path.join(root, f)), reverse=True)
...
def moderation():
    ...
    for root, _, files in os.walk(PENDING_DIR):
        for f in files:
            if f.endswith(".pdf"):
                fichiers_pending.append(os.path.relpath(os.path.join(root, f), PENDING_DIR))
    fichiers_pending.sort(key=lambda f: os.path.getmtime(os.path.join(PENDING_DIR, f)), reverse=True)
...
def valider(filename):
    ...
    shutil.move(src, dst)
    flash(f"Fichier validé : {filename}")
    return redirect(url_for("moderation"))
...
def supprimer(filename):
    ...
    if os.path.exists(chemin):
        os.remove(chemin)
        flash(f"Supprimé : {filename}")
    return redirect(url_for("moderation"))

@app.route("/supprimer_valide/<path:filename>")
def supprimer_valide(filename):
    if not session.get("moderateur"):
        return redirect(url_for("moderation"))
    chemin = os.path.join(PDF_DIR, filename)
    if os.path.exists(chemin):
        os.remove(chemin)
        flash(f"Supprimé (validé) : {filename}")
    return redirect(url_for("moderation"))
